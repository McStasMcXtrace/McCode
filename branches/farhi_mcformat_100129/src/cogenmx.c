...
* ##tc2
* ##tr1
* ##nx             Xray state (position, wavevector, phase, polarisation, and weight).
* ##ny
* ##nz
* ##nkx
* ##nky
...
* ##NCounter        Incremented each time an Xray is entering the component
* ##AbsorbProp      single counter for removed events in PROP calls
* ##comp_storein    Positions of xray entering each comp (loc. coords)
...
* 1. Header file #include - "mcxtrace-r.h" for declarations for the
*    mcstas runtime.
...
* X. Declaration of variables for xray state.
...
cogen_decls(struct instr_def *instr)
{
  ...
  /* 6. Table to store xray states when entering each component */
  cout("/* Xray state table at each component input (local coords) */");
  cout("/* [x, y, z, kx, ky, kz, phi, Ex, Ey, Ez, p] */");
  ...
  cout("/* Flag true when previous component acted on the xray (SCATTER) */");
  ...
  /* 12. Xray state. */
  coutf("MCNUM %snx, %sny, %snz, %snkx, %snky, %snkz, %snphi, "
        "%snEx, %snEy, %snEz, %snp;",
...
*   ABSORB xrays are sent to label absorbComp at the end of component
*          and next comp in GROUP is tested.
*   JUMP:  sends xray to the JumpTrace labels, either with condition
...
cogen_trace(struct instr_def *instr)
{
  ...
  coutf("void %sraytrace(void) {", ID_PRE);
  /* Local xray state. */
  cout("  /* Copy xray state to local variables. */");
  coutf("  MCNUM %snlx = %snx;", ID_PRE, ID_PRE);
  coutf("  MCNUM %snly = %sny;", ID_PRE, ID_PRE);
  coutf("  MCNUM %snlz = %snz;", ID_PRE, ID_PRE);
  coutf("  MCNUM %snlkx = %snkx;", ID_PRE, ID_PRE);
  coutf("  MCNUM %snlky = %snky;", ID_PRE, ID_PRE);
  coutf("  MCNUM %snlkz = %snkz;", ID_PRE, ID_PRE);
  coutf("  MCNUM %snlphi = %snphi;", ID_PRE, ID_PRE);
  coutf("  MCNUM %snlEx = %snEx;", ID_PRE, ID_PRE);
  coutf("  MCNUM %snlEy = %snEy;", ID_PRE, ID_PRE);
  coutf("  MCNUM %snlEz = %snEz;", ID_PRE, ID_PRE);
  coutf("  MCNUM %snlp = %snp;", ID_PRE, ID_PRE);
  cout("");

  /* Debugging (initial state). */
  coutf("  %sDEBUG_ENTER()", ID_PRE);
  coutf("  %sDEBUG_STATE(%snlx, %snly, %snlz, %snlkx, %snlky, %snlkz,"
      "%snlphi,%snlEx,%snlEy,%snlEz, %snlp)",
        ID_PRE, ID_PRE, ID_PRE, ID_PRE, ID_PRE, ID_PRE, ID_PRE,
  ...
  while((comp = list_next(liter)) != NULL)
  {
    ...
    static char *statepars_names[11] =
      {
        "nlx", "nly", "nlz", "nlkx", "nlky", "nlkz",
        "nlphi", "nlEx", "nlEy","nlEz", "nlp"
      };
    int i;
    ...
    coutf("    &%snlkx, &%snlky, &%snlkz);", ID_PRE, ID_PRE, ID_PRE);
//    coutf("    &%snlphi, &%snlEx, &%snlEy);", ID_PRE, ID_PRE, ID_PRE);
    if(instr->polarised)
      coutf("  %scoordschange_polarisation("
            "%srotr%s, &%snlEx, &%snlEy, &%snlEz);",
    ...
    /* store xray state in mccomp_storein */
    if (!comp->split)
    coutf("  STORE_XRAY(%i,%snlx, %snly, %snlz, %snlkx,"
        "%snlky,%snlkz,%snlphi,%snlEx,%snlEy, %snlEz, %snlp);",
          comp->index, ID_PRE, ID_PRE, ID_PRE, ID_PRE, ID_PRE, ID_PRE,
          ID_PRE, ID_PRE, ID_PRE, ID_PRE, ID_PRE, ID_PRE);
    else {
      /* spliting: store first time, then restore xray */
      char *exp=exp_tostring(comp->split); /* number of splits */
      coutf("  if (!%sSplit_%s) {                   /* STORE only the first time */", ID_PRE, comp->name);
      coutf("    if (floor(%s) > 1) p /= floor(%s); /* adapt weight for SPLITed xray */", exp, exp);
      coutf("    STORE_XRAY(%i,%snlx, %snly, %snlz, %snlkx,"
          "%snlky,%snlkz,%snlphi,%snlEx,%snlEy, %snlEz, %snlp);",
          comp->index, ID_PRE, ID_PRE, ID_PRE, ID_PRE, ID_PRE, ID_PRE,
          ID_PRE, ID_PRE, ID_PRE, ID_PRE, ID_PRE, ID_PRE);
      coutf("  } else {");
      coutf("    RESTORE_XRAY(%i,%snlx, %snly, %snlz, %snlkx,"
          "%snlky,%snlkz,%snlphi,%snlEx,%snlEy, %snlEz, %snlp); }",
          comp->index, ID_PRE, ID_PRE, ID_PRE, ID_PRE, ID_PRE, ID_PRE,
          ID_PRE, ID_PRE, ID_PRE, ID_PRE, ID_PRE, ID_PRE);
    ...
    if (comp->group) {
      ...
      if (strcmp(comp->name, comp->group->last_comp)) {
        ...
        coutf("  if (!%sGroup%s) /* restore xray if was not scattered in GROUP yet */", ID_PRE, comp->group->name);
        coutf("    { RESTORE_XRAY(%i,%snlx, %snly, %snlz, %snlkx,"
            "%snlky,%snlkz,%snlphi,%snlEx,%snlEy, %snlEz, %snlp); }",
          comp->index, ID_PRE, ID_PRE, ID_PRE, ID_PRE, ID_PRE, ID_PRE,
          ID_PRE, ID_PRE, ID_PRE, ID_PRE, ID_PRE);
        ...
        coutf("  if (!%sGroup%s) ABSORB; /* absorb xrays non scattered in GROUP */", ID_PRE, comp->group->name);
      }
    }
  ...
  /* Absorbing xrays - goto this label to skip remaining components. End of TRACE */
  ...
  /* Copy back xray state to global variables. */
  ...
  cout("  /* Copy xrays state to global variables. */");
  coutf("  %snx = %snlx;", ID_PRE, ID_PRE);
  coutf("  %sny = %snly;", ID_PRE, ID_PRE);
  coutf("  %snz = %snlz;", ID_PRE, ID_PRE);
  coutf("  %snkx = %snlkx;", ID_PRE, ID_PRE);
  coutf("  %snky = %snlky;", ID_PRE, ID_PRE);
  coutf("  %snkz = %snlkz;", ID_PRE, ID_PRE);
  coutf("  %snphi = %snlphi;", ID_PRE, ID_PRE);
  coutf("  %snEx = %snlEx;", ID_PRE, ID_PRE);
  coutf("  %snEy = %snlEy;", ID_PRE, ID_PRE);
  coutf("  %snEz = %snlEz;", ID_PRE, ID_PRE);
...
cogen_finally(struct instr_def *instr)
{
  ...
  while(comp = list_next(liter))
  {
    ...
    coutf("    if (!%sNCounter[%i]) "
      "fprintf(stderr, \"Warning: No xray could reach Component[%i] %s\\n\");",
