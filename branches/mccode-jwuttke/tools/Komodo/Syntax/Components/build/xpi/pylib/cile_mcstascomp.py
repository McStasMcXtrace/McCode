...
"""A Code Intelligence Language Engine for the McStasCOMP language.
...
Module Usage:
    from cile_mcstascomp import scan
    mtime = os.stat("bar.mcstascomp")[stat.ST_MTIME]
    content = open("bar.mcstascomp", "r").read()
    scan(content, "bar.mcstascomp", mtime=mtime)
"""
...
class McStasCOMPCILEError(CILEError):
    pass
...
log = logging.getLogger("cile.mcstascomp")
...
def scan_buf(buf, mtime=None, lang="McStasCOMP"):
    """Scan the given McStasCOMPBuffer return an ElementTree (conforming
    to the CIX schema) giving a summary of its code elements.
    
    @param buf {McStasCOMPBuffer} is the McStasCOMP buffer to scan
    ...
    """
    # Dev Notes:
    # - This stub implementation of the McStasCOMP CILE return an "empty"
    #   summary for the given content, i.e. CIX content that says "there
    #   are no code elements in this McStasCOMP content".
    # - Use the following command (in the extension source dir) to
    #   debug/test your scanner:
    #       codeintel scan -p -l McStasCOMP <example-McStasCOMP-file>
    ...
    # Dev Note:
    # This is where you process the McStasCOMP content and add CIX elements
    ...
    #   Use the "codeintel html -b <example-McStasCOMP-file>" command as
    #   a debugging tool.
    # - "buf.accessor.text" is the whole content of the file. If you have
    #   a separate tokenizer/scanner tool for McStasCOMP content, you may
    #   want to use it.
