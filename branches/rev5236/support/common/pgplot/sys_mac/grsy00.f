...
C               undefined [TJP].
C 21-Jan-1995 Modified to work on Mac with MPW Fortran 2.1 (Ie. Split grfont.dat into
C 3 records, since maximum record length is 32,767).
C-----------------------------------------------------------------------
      CHARACTER*(*) DEFNAM
...
C
     ...
     2      STATUS='OLD', IOSTAT=IER)
      IF (IER.EQ.0) Then
C MPW Fortran 2.1 has a maximum record size of 32767 bytes for multiple items in an 
C unformatted I/O statement.  So put NC1, NC2, LOC, and INDEX in one record (The total
C length will be 4 * (1 + 1 + 1 + 3000) = 12,012 bytes.  Split Index in two put them each
C in a record.  The length will be 2 * 13500 = 27,000.
	      READ (UNIT=FNTFIL, IOSTAT=IER) NC1,NC2,NC3,INDEX
          Read (UNIT=FNTFIL, IOSTAT=IER) (Buffer(I), I = 1,13500)
	      Read (UNIT=FNTFIL, IOSTAT=IER) (Buffer(I), I = 13501,27000)
	  End If
     ...
     :          //'the location of the PGPLOT grfont.dat file,')
	      Call GRWARN('Or put grfont.dat in your current directory.')
      END IF
      RETURN
      END
