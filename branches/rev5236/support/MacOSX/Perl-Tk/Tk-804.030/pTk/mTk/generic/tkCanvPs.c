...
static Tk_ConfigSpec configSpecs[] = {
    {TK_CONFIG_HASHVAR, "-colormap", (char *) NULL, (char *) NULL,
...
TkCanvPostscriptCmd(canvasPtr, interp, argc, argv)
    ...
    CONST84 Tcl_Obj *CONST *objv;	/* Argument strings.  Caller has
    ...
    XColor *colorPtr;			/* Information about color. */
{
	...
	Tcl_Obj * cmdString;
	Tcl_Obj *name = Tcl_NewStringObj(Tk_NameOfColor(colorPtr),-1);

	cmdString = Tcl_ObjGetVar2(interp, psInfoPtr->colorVar, name,0);
        Tcl_DecrRefCount(name);

	if (cmdString != NULL) {
	    Tcl_AppendResult(interp, Tcl_GetString(cmdString), "\n", (char *) NULL);
	    return TCL_OK;
	}
    }
					 ...
					 * is to be printed. */
{
	...
	name = Tcl_NewStringObj(Tk_NameOfFont(tkfont),-1);
	list = Tcl_ObjGetVar2(interp, psInfoPtr->fontVar, name, 0);
        Tcl_DecrRefCount(name);
	if (list != NULL) {
	    ...
	    if (Tcl_GetDoubleFromObj(interp, objv[1], &size) != TCL_OK) {
		goto badMapEntry;
	    }
