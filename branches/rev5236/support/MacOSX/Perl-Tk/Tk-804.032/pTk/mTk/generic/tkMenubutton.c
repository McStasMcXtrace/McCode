...
static Tk_OptionSpec optionSpecs[] = {
    ...
    {TK_OPTION_OBJ, "-height", "height", "Height",
    ...
    {TK_OPTION_WINDOW, "-menu", "menu", "Menu",
	...
	DEF_MENUBUTTON_TEXT_VARIABLE,
        -1, Tk_Offset(TkMenuButton, textVarName), TK_OPTION_NULL_OK, 0, 0},
    {TK_OPTION_INT, "-underline", "underline", "Underline",
	DEF_MENUBUTTON_UNDERLINE, -1, Tk_Offset(TkMenuButton, underline),
         0, 0, 0},
    {TK_OPTION_OBJ, "-width", "width", "Width",
...
static char *		MenuButtonTextVarProc _ANSI_ARGS_((
			    ...
			    Var name1, CONST char *name2, int flags));
    ...
    char *memPtr;		/* Info about button widget. */
{
	...
	Lang_UntraceVar(mbPtr->interp, mbPtr->textVarName,
    ...
    Tcl_Obj *CONST objv[];	/* Arguments. */
{
    ...
    if (mbPtr->textVarName != NULL) {
	Lang_UntraceVar(interp, mbPtr->textVarName,
		TCL_GLOBAL_ONLY|TCL_TRACE_WRITES|TCL_TRACE_UNSETS,
		MenuButtonTextVarProc, (ClientData) mbPtr);
    }
	...
	if ((mbPtr->bitmap != None) || (mbPtr->image != NULL)) {
	    if (Tk_GetPixelsFromObj(interp, mbPtr->tkwin, mbPtr->widthString,
                    &mbPtr->width) != TCL_OK) {
                widthError:
	        Tcl_AddErrorInfo(interp, "\n    (processing -width option)");
		continue;
	    }
	    if (Tk_GetPixelsFromObj(interp, mbPtr->tkwin, mbPtr->heightString,
		    &mbPtr->height) != TCL_OK) {
	        heightError:
	    ...
	    if (Tcl_GetIntFromObj(interp, mbPtr->widthString, &mbPtr->width)
		    != TCL_OK) {
	        goto widthError;
	    }
	    if (Tcl_GetIntFromObj(interp, mbPtr->heightString, &mbPtr->height)
		    != TCL_OK) {
	        goto heightError;
	    }
	}
    ...
    if ((mbPtr->image == NULL) && (mbPtr->bitmap == None)
      ...
      value = Tcl_GetString(Tcl_ObjGetVar2(interp, mbPtr->textVarName, NULL, TCL_GLOBAL_ONLY));
      if (value == NULL) {
	  Tcl_Obj *temp = Tcl_NewStringObj(mbPtr->text,-1);
	  Tcl_ObjSetVar2(interp, mbPtr->textVarName, NULL, temp,
		     TCL_GLOBAL_ONLY);
      ...
      Lang_TraceVar(interp, mbPtr->textVarName,
		   TCL_GLOBAL_ONLY|TCL_TRACE_WRITES|TCL_TRACE_UNSETS,
		   MenuButtonTextVarProc, (ClientData) mbPtr);
    }
