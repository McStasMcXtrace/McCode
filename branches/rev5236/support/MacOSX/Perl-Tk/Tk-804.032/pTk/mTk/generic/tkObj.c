...
#define SIMPLE_PIXELREP(objPtr)                         \
    (TclObjInternal(objPtr)->twoPtrValue.ptr2 == 0)

#define SET_SIMPLEPIXEL(objPtr, intval)                 \
    TclObjInternal(objPtr)->twoPtrValue.ptr1 = (VOID *) (intval);       \
    TclObjInternal(objPtr)->twoPtrValue.ptr2 = 0

#define GET_SIMPLEPIXEL(objPtr)                         \
    ((int) TclObjInternal(objPtr)->twoPtrValue.ptr1)

#define SET_COMPLEXPIXEL(objPtr, repPtr)                \
    TclObjInternal(objPtr)->twoPtrValue.ptr1 = 0;               \
    TclObjInternal(objPtr)->twoPtrValue.ptr2 = (VOID *) repPtr

#define GET_COMPLEXPIXEL(objPtr)                        \
    ((PixelRep *) TclObjInternal(objPtr)->twoPtrValue.ptr2)
...
static void
DupPixelInternalRep(srcObjPtr, copyObjPtr)
    register Tcl_Obj *srcObjPtr;        /* Object with internal rep to copy. */
    register Tcl_Obj *copyObjPtr;       /* Object with internal rep to set. */
{
    PixelRep *oldPtr, *newPtr;

    copyObjPtr->typePtr = srcObjPtr->typePtr;

    if (SIMPLE_PIXELREP(srcObjPtr)) {
	SET_SIMPLEPIXEL(copyObjPtr, GET_SIMPLEPIXEL(srcObjPtr));
    } else {
	oldPtr = GET_COMPLEXPIXEL(srcObjPtr);
	newPtr = (PixelRep *) ckalloc(sizeof(PixelRep));
	newPtr->value = oldPtr->value;
	newPtr->units = oldPtr->units;
	newPtr->tkwin = oldPtr->tkwin;
	newPtr->returnValue = oldPtr->returnValue;
	SET_COMPLEXPIXEL(copyObjPtr, newPtr);
    }
}
    ...
    Tcl_Obj *objPtr;            /* The object to convert. */
{
    ...
    TclObjSetType(objPtr,&pixelObjType);
...
static void
DupMMInternalRep(srcObjPtr, copyObjPtr)
    register Tcl_Obj *srcObjPtr;        /* Object with internal rep to copy. */
    register Tcl_Obj *copyObjPtr;       /* Object with internal rep to set. */
{
    MMRep *oldPtr, *newPtr;
    ...
    copyObjPtr->typePtr = srcObjPtr->typePtr;
    oldPtr = (MMRep *) srcObjPtr->internalRep.otherValuePtr;
    newPtr = (MMRep *) ckalloc(sizeof(MMRep));
    newPtr->value = oldPtr->value;
    newPtr->units = oldPtr->units;
    newPtr->tkwin = oldPtr->tkwin;
    newPtr->returnValue = oldPtr->returnValue;
    copyObjPtr->internalRep.otherValuePtr = (VOID *) newPtr;
}
    ...
    register Tcl_Obj *objPtr;   /* The object to convert. */
{
    ...
    TclObjSetType(objPtr,&windowObjType);
...
static void
DupWindowInternalRep(srcObjPtr, copyObjPtr)
    register Tcl_Obj *srcObjPtr;
    register Tcl_Obj *copyObjPtr;
{
    register WindowRep *oldPtr, *newPtr;

    oldPtr = srcObjPtr->internalRep.otherValuePtr;
