...
Tk_CreateBinding(interp, bindingTable, object, eventString, command, append)
    ...
    CONST char *eventString;	/* String describing event sequence that
				 * triggers binding. */
    Tcl_Obj *command;		/* Contains Tcl command to execute when
				 * binding triggers. */
				 ...
				 * replaced. */
{
	...
	callback = LangMakeCallback(command);
    }
    if (old != NULL) {
	ckfree(old);
    }
    psPtr->eventProc = LangEventCallback;
    psPtr->freeProc  = (TkBindFreeProc *) LangFreeCallback;
    psPtr->clientData = (ClientData) callback;
    return eventMask;
}
					 ...
					 * that triggers binding. */
{
	...
	return Tcl_NewStringObj((char *) psPtr->clientData, -1);
    }
    if (psPtr->eventProc == LangEventCallback) {
	/* REFCNT is not incremented so mimic what tkConfig.c does */
	return LangCallbackObj((LangCallback *) psPtr->clientData);
    }
					 ...
					 * to check for a matching binding. */
{
	...
	Tk_ChangeScreen(interp, dispPtr->name, screenPtr->curScreenIndex);
    }
        ...
        doAtom:
         {Tk_Window tkwin;
          ...
          if ((tkwin = Tk_EventWindow(eventPtr)))
           string = (char *) Tk_GetAtomName(tkwin, (Atom) number);
          else
           sprintf(string = numStorage, "0x%x", number);
         }
         goto doNumeric;
