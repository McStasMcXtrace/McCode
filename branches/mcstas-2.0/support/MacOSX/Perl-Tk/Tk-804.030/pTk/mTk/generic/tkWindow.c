...
static Tcl_Obj * name;
static Tcl_Obj * display;
static Tcl_Obj * geometry;
static Tcl_Obj * colormap = NULL;
static Tcl_Obj * visual = NULL;
static Tcl_Obj * use = NULL;
static int rest = 0;
    ...
    int *screenPtr;		/* Where to store screen number. */
{
	...
	if (dispPtr == NULL) {
	    ...
	    /* If X can do UTF-8 for us avoid locale/encode issues
	       by defaulting to using input methods that way.
	     */
				 ...
				 * form "prog instance". */
{
	...
	if (cmdPtr->cmdProc != NULL) {
	    Tcl_CreateObjCommand(interp, cmdPtr->name, cmdPtr->cmdProc,
    ...
    Tk_Window tkwin;		/* Window to destroy. */
{
    ...
    if (winPtr->window != None) {
#if defined(MAC_TCL) || defined(MAC_OSX_TK) || defined(__WIN32__) || defined(__PM__)
    ...
    Tcl_Interp *interp;		/* Interpreter to initialize. */
{
    ...
    argvt[1] = ".";
    argvt[2] = "-class";
    argvt[3] = Tcl_DStringValue(&class);
    argc = 4;
    if (display != NULL) {
	argvt[argc] = "-screen";
	argvt[argc+1] = display;
	argc += 2;
