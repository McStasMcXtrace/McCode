					 ...
					 * within this application. */
{
	...
	for (numTries = 0; numTries < 10; numTries++) {
	    grabResult = XGrabPointer(dispPtr->display, winPtr->window,
		    ...
		    winPtr->atts.cursor, CurrentTime);
	...
	if (eventPtr->type == ButtonPress) {
		...
		if (!(dispPtr->grabFlags & GRAB_GLOBAL)) {	/* Note 6. */
		    ...
		    if (XGrabPointer(dispPtr->display,
			    ...
			    GrabModeAsync, GrabModeAsync, None, dispPtr->eventualGrabWinPtr->atts.cursor,
			    CurrentTime) == 0) {
			EatGrabEvents(dispPtr, serial);
				 ...
				 * generate enter events. */
{
    ...
    event.xcrossing.root = RootWindow(winPtr->display,
	    winPtr->screenNum);
    event.xcrossing.time = TkCurrentTime(winPtr->dispPtr, False);
    /* FALSE because we want a real time (even an old one) not CurrentTime */
