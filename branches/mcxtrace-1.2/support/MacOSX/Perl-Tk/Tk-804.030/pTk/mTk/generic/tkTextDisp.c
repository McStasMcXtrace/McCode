				 ...
				 * argv[1] is "yview". */
{
	...
	if (!strchr(Tcl_GetString(objv[2+pickPlace]),'.') &&
	    Tcl_GetIntFromObj(interp, argv[2+pickPlace], &lineNum) == TCL_OK) {
				 ...
				 * argv[1] is "scan". */
{
    ...
    if (Tcl_GetIntFromObj(interp, argv[4], &y) != TCL_OK) {
	return TCL_ERROR;
    }
					 ...
					 * system as chunkPtr->x. */
{
    ...
    return MeasureChars(chunkPtr->stylePtr->sValuePtr->tkfont, ciPtr->chars,
	    chunkPtr->numBytes, chunkPtr->x, x, 0, &endX);
						/* CHAR OFFSET */
}
					 ...
					 * character, in pixels. */
{
	...
	*widthPtr = maxX - *xPtr;
        ...
        Tcl_UniChar ch;
	char *p = ciPtr->chars + byteIndex;
	MeasureChars(chunkPtr->stylePtr->sValuePtr->tkfont,
		p, Tcl_UtfToUniChar(p, &ch), *xPtr, -1, 0, widthPtr);
	if (*widthPtr > maxX) {
	    *widthPtr = maxX - *xPtr;
	} else {
	    *widthPtr -= *xPtr;
