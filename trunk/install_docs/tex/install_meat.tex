...
\item{\texttt{mcstas-1.11-i686.deb} and \texttt{mcstas-1.11-x64.deb}\\Binary Debian GNU/Linux packages for 32 bit and 64 bit Intel respectively, currently built on Debian stable.
...
\\\verb+deb http://apt.mcstas.org binary/+ (32 bit package) or
\\\verb+deb http://apt.mcstas.org binary64/+ (64 bit package)}
\item{As root execute the command:\\\verb+apt-get update+}
\item{As root execute the command:\\\verb+apt-get install mcstas+}
\end{enumerate}
Using our Debian package and updating your operating system will ensure automatic upgrade of McStas in case of
a new release.
