...
#===============#
    def ldapQuery(self, auth_dn, auth_pw, query):
        ...
        pipe = PIPE
        self.query_num += 1
        log_str  = cn + " QUERY with: ldapsearch -LLL -b dc=fysik,dc=dtu,dc=dk -D" 
        log_str += auth_dn + "-w PASSWORD " + query                         # TEMPLATE LINE
        self.log(log_str)
        try:
            fid  = Popen(["ldapsearch", "-LLL", "-b", "dc=fysik,dc=dtu,dc=dk", 
                          "-D", auth_dn, "-w", auth_pw, query],                         # TEMPLATE LINE
                         stdout=pipe,
                         stderr=pipe)
...
#==========================#
    def ldapAdminGroupQuery(self, auth_cn):
        ...
        self.log("%s AUTHORITY ACCESS QUERY with: ldapsearch -LLL -b "+
                 "ou=groups,dc=fysik,dc=dtu,dc=dk -D "+
                 "cn=DummyUser,ou=person,dc=fysik,dc=dtu,dc=dk -w DummyPW %s" 
                 % (cn, query) ) # TEMPLATE LINE
        pipe = PIPE
        try:
            fid = Popen(
                ["ldapsearch", "-LLL", "-b", "dc=fysik,dc=dtu,dc=dk", "-D", 
                 "cn=DummyUser,ou=person,dc=fysik,dc=dtu,dc=dk", "-w", "DummyPW", query],  #TEMPLATE LINE
                stdout=pipe,
                stderr=pipe)
            stdout,stderr = fid.communicate()
            bill = stdout
...
#=====================#
    def ldapAuthenticate(self, auth_cn, auth_pw):
        dn = "cn=%s" + auth_cn + "ou=person,dc=fysik,dc=dtu,dc=dk"                         # TEMPLATE LINE
