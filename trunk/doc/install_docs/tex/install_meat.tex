...
\item{Optionally, install FreeWRL from \verb+http://freewrl.sourceforge.net/download.html+ or WhiteDune with \\\verb+apt-get install whitedune+.}
...
\item{If you have acces to Matlab(R), install it prior to McStas, so that the configuration can detect it.}
\item{As root, issue the command \\\verb+dpkg -i mcstas-2.0-amd64.deb+ (or \\\verb+dpkg -i mcstas-2.0-i386.deb+ on 32 bit systems). This later step may be replaced by a ./configure; make; make install procedure after extraction of the Mctas tarball (see below)}
...
We hope to make a so-called \emph{apt repository} available in the future, which will ensure automatic upgrade of McStas in case of a new release.
