...
%%\begin{figure}[htb!]
%%  \begin{center}
%%    \includegraphics[width=0.55\textwidth]{figures/mxdisplay_Scilab.ps}
%%    \includegraphics[width=0.25\textwidth]{figures/mxdisplay_Scilab_dialog.ps}
%%  \end{center}
%%\caption{Output from \texttt{mxdisplay} with Scilab backend. Display
...
%photon event files using components
%\begin{itemize}
...
%This also equivalently stands for Scilab (using the \verb+get_+\textit{sim file} function, the 'exec' call is required in order to compile the code)
%\begin{verbatim}
%    scilab> exec('mcxtrace.sci', -1); s=get_mcxtrace();
%    scilab> exec('mcxtrace.sci', -1); s=get_mcxtrace('plot')
...
%See section~\ref{s:mxplot} for an other way of plotting simulation results using the \verb+mxplot+ front-end.\index{Tools!mxplot} \index{Tools!mxplot}
...
%\item Divide simulation into parts using a file for saving or generating photons
...
section~\ref{s:mxplot}. \index{Tools!mxplot}
...
\item The \verb+mxrun --test+ command will test your \MCX\ installation, accuracy and plotter. \index{Installing}
\end{itemize}
...
The drawing back-end program may be selected among PGPLOT, VRML, Matlab and Scilab using either the -p\textit{PLOTTER} option or using the current \verb+MCSTAS_FORMAT+ environment variable. \index{Environment variable!MCSTAS\_FORMAT}
...
\subsection{Translating and merging \MCX\ results files (all text formats)}
\label{s:mxformat}
\index{Tools!mxformat|textbf}
...
Once the distributed simulation have been completed, you may merge their results using \verb+mxformat+ (see section \ref{s:mxformat}) \index{Tools!mxformat} in order to obtain a set of files just as if it had been executed on a single machine.
