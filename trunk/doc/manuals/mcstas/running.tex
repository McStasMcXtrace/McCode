...
The available format list is obtained using the \verb+name.out --help+ option. \index{Tools!PGPLOT}
\index{Tools!Matlab}\index{Data formats} \MCS can presently generate the \MCS /PGPLOT and the NeXus format. 
...
\begin{lstlisting}
    idl> s=mcstas()
    idl> s=mcstas(/plot)
\end{lstlisting} \index{Tools!IDL}
...
variable. \index{Environment variable!MCSTAS\_FORMAT}.
...
\index{Data formats} \index{Grid computing}
...
The available data formats are the legacy McStas (McCode) format, and the HDF/NeXus binary when the needed libraries are installed. \index{Environment variable!MCSTAS\_FORMAT}
