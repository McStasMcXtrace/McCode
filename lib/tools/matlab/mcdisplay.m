...
uicontrol(h,'style','text','units','normalized','position',...
	  [5/11 0.05 1/11 0.05],'string','Trajectories to keep','fontsize',10);
% 'Exit button'
...
% 'Lock button'
uicontrol(h,'style','pushbutton','units','normalized','position',...
	  [6/11 0 1/11 0.05],'string','Lock','callback', ...
	  'wait(INSTRUMENT.fig);','fontsize',10,'tag','Lock');

% 'Unlock button'
uicontrol(h,'style','pushbutton','units','normalized','position',...
	  [6/11 0.05 1/11 0.05],'string','Unlock','callback', ...
...
uicontrol(h,'style','edit','units','normalized','position',...
	  [5/11 0 1/11 0.05],'string','1','userdata',1,'callback', ...
...
% 'Timeout label'
uicontrol(h,'style','text','units','normalized','position',...
	  [7/11 0.05 1/11 0.05],'string','Plot timeout (sec)','fontsize',10);

% 'Timeout edit field'
uicontrol(h,'style','edit','units','normalized','position',...
	  [7/11 0 1/11 0.05],'string','0.2','userdata',0.2,'callback', ...
	  'CheckTimeout;','fontsize',10,'tag','Timeout');

% 'Component list label'
uicontrol(h,'style','text','units','normalized','position',...
	  [0/11 0.05 1/11 0.05],'string','Comp."s:','fontsize',10);

% 'Start component listbox'
uicontrol(h,'style','listbox','units','normalized','position',...
	  [1/11 0 2/11 0.1],'string','start','tag','startcomp', ...
	  'fontsize',10);

% 'End component listbox'
uicontrol(h,'style','listbox','units','normalized','position',...
	  [3/11 0 2/11 0.1],'string','end','tag','endcomp','fontsize',10);
