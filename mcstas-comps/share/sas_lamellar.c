...
#define IQ_KERNEL_NAME lamellar_Iq
#define IQ_PARAMETERS sld, solvent_sld, thickness
...
#define IQXY_KERNEL_NAME lamellar_Iqxy
#define IQXY_PARAMETERS sld, solvent_sld, thickness
...
#define IQXY_PARAMETER_DECLARATIONS float sld, float solvent_sld, float thickness
...
float Iq(float q, IQ_PARAMETER_DECLARATIONS) {
    ...
    const float qsq = q*q;
    // Original expression
    //return 4.0e-4f*M_PI*sub*sub/qsq * (1.0f-cos(q*thickness)) / (thickness*qsq);
    // const float alpha = fmod(q*thickness+0.1f, 2.0f*M_PI)-0.1f;
    // Use small angle fix 1-cos(theta) = 2 sin^2(theta/2)
    const float sinq2 = sin(0.5f*q*thickness);
    return 4.0e-4f*M_PI*sub*sub/qsq * 2.0f*sinq2*sinq2 / (thickness*qsq);
