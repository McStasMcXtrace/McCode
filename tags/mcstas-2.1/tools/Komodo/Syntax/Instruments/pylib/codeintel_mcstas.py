...
lang = "McStas"
log = logging.getLogger("codeintel.mcstas")
#log.setLevel(logging.DEBUG)


# These keywords are copied from "McStas-mainlex.udl" - be sure to keep both
# of them in sync.
keywords = [
    # Keywords
]
...
# lexers. You must define a Lexer class for lang McStas. If Komodo's
# scintilla lexer for McStas is UDL-based, then this is simply:
#
#   from codeintel2.udl import UDLLexer
#   class McStasLexer(UDLLexer):
#       lang = lang
#
# Otherwise (the lexer for McStas is one of Komodo's existing C++ lexers
...
#   class McStasLexer(Lexer):
#       lang = lang
#       def __init__(self):
#           self._properties = SilverCity.PropertySet()
#           self._lexer = SilverCity.find_lexer_module_by_id(ScintillaConstants.SCLEX_MCSTAS)
#           self._keyword_lists = [
...
# examples of usage.
class McStasLangIntel(LangIntel):
    lang = lang
...
# McStas content and returns a description of the code in that file.
# See "cile_mcstas.py" for more details.
#
# The CILE Driver is a class that calls this CILE. If McStas is
...
class McStasCILEDriver(UDLCILEDriver):
    lang = lang

    def scan_purelang(self, buf):
        import cile_mcstas
        return cile_mcstas.scan_buf(buf)
...
def register(mgr):
    ...
    mgr.set_lang_info(
        lang,
        silvercity_lexer=McStasLexer(),
        buf_class=McStasBuffer,
        langintel_class=McStasLangIntel,
        import_handler_class=None,
        cile_driver_class=McStasCILEDriver,
