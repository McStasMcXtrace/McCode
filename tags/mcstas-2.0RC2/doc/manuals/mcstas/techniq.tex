...
simulation, see section \cite{s:weighttransform}. For convenience, the value of $p$ is updated (within each component)
...
transformation, in our notation written as $f_{\rm MC}=P, \pi_j=1$. To
...
within the Debye-Scherrer cone. This weight transformation is much more complex than described above, but still boils down to obeying the master transformation rule \cite{probrule}.
...
This implies $f_{\rm MC}(\Delta\Omega) = 1$. However, if the physical
events are distributed uniformly over the unit sphere, we would have
$P(\Delta\Omega) = \Delta\Omega / (4\pi)$, according to Eq.~(\ref{probrule}).
