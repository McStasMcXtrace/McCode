...
def display_single(FileStruct):
    ...
    if type == 'array_1d':
        ...
        I, E, N = FileStruct['values'].split()
        Title += '\nI=%s E=%s N=%s' % (I, E, N)
        colorbar()

    xlabel(FileStruct['xlabel'],
           fontsize=FileStruct['FontSize'] * 1.5, fontweight='bold')
...
def read_monitor(path):
    ...
    filestruct = {}
    for line in filter(is_header, open(path)):
        # Field name and data
        k, v = line.lstrip('# ').split(':', 1)
        filestruct[k.strip()] = v.strip()
    filestruct['data'] = loadtxt(path)
    filestruct['fullpath'] = path
    print "Loading " + path
...
def get_monitor(FS, j):
    ...
    FSsingle = {'xlimits': FS['xlimits'],
                'data': data,
                'component': vars[j + 1],
                'values': '',
                'type': 'array_1d(100)',
                'xlabel': FS['xlabel'], 'ylabel': FS['ylabel'],
                'File': 'Scan', 'title': '', 'FontSize': 6}
    return FSsingle
    # end get_monitor
...
def keypress(event):
    ...
    Handle key shortcut in a window
      ...
      output: None
    """
    from pylab import close, figure, text, title, show

    # Ignore non-key events (mouse)
    if event.key is None:
        return

    formats = {'p': 'ps', 'd': 'pdf', 'n': 'png', 'j': 'jpg'}

    event.key = event.key.lower()
    if event.key in ('q', 'x'):
        close('all')
    elif event.key in formats:
        dumpfile(formats[event.key])
    elif event.key in ('o', 'l'):
        'toggle lin/log'
        ...
        title(basename(sys.argv[0]) + ": plotting tool for McCode data set",
              fontweight='bold')
        show()
    elif event.key in ('right', 'pagedown'):
        'show next scan step/monitor'
        display_scanstep(File, +1)
    elif event.key in ('left', 'backspace', 'pageup'):
...
def display(path, comment):
    ...
    # FIX: make the path to path absolute to always get a non-empty dirname.
    path = os.path.abspath(path)

    if isdir(path):
        # dirname given, assume mcstas.sim in that dir.
        path = os.path.join(path, 'mcstas.sim')
    ...
    if not isfile(path):
        print basename(sys.argv[0]) + ": Dataset " + File + " does not exist!"
        exit()
    ...
    SimFile = filter(isBegin, open(path))
    Datfile = 0
    if SimFile == []:
        FS = read_monitor(path)
        type = FS['type'].split('(')[0].strip()
        if type != 'multiarray_1d':
            FS['FontSize'] = 8
            FS = display_single(FS)
            if options.Format != None:
                savefig(path + "." + options.Format)
                print "Saved " + File + "." + options.Format
            show()
            exit()

    # Get filenames from the sim file
    MonFiles = filter(isCompFilename, open(path).readlines())
    L = len(MonFiles)
    ...
    if L == 0:
        ...
        if Datfile == 0:
            ...
            Scanfile = filter(isFilename, open(path));
            Scanfile = Scanfile[0].split(': ')
            Scanfile = os.path.join(dirname(path), Scanfile[1].strip())
...
#            except ImportError:
    ...
    if options.Format != None:
        ...
        from pylab import get_current_fig_manager
        tb = get_current_fig_manager().toolbar
        tb.set_message('File: ' + File + comment +
                       ' mcplot: Keys: h=help [goc|x|pdnj] ' + File + comment)
...
if __name__ == "__main__":
    ...
    prof = Profile("mcplot.prof")
    try:
        prof.runcall(display, File, "")
    except:
        prof.close()
    # End of __main__
