...
the x-rays which have negative propagation times are removed automatically.
\begin{itemize}
...
\item {\bf PROP\_Z0}. Propagates the x-ray to the $z=0$ plane,
  by adjusting $(x,y,z)$ and $t$ accordingly from knowledge of the
  x-ray wavevector $(k_x,k_y,k_z)$.
  If the propagation time is negative, the x-ray is absorbed, except if a \verb+ALLOW_BACKPROP;+ preceeds it.
...
\item {\bf PROP\_X0,PROP\_Y0}. These macros are analogous to \verb+PROP_Z0+ except they propaget to the $x=0$ and $y=0$ planes respectively.

\item {\bf PROP\_DL}$(dl)$. Propagates the x-ray by the length $dl$, adjusting $(x,y,z)$ and $\phi$ accordingly,
...
\item {\bf ALLOW\_BACKPROP}. Indicates that the next propagation routine
  ...
  are found. Further propagations are not affected.\index{Removed x-ray events}
...
\item {\bf STORE\_XRAY}$(index, x, y, z, kx, ky, kz, phi$, $Ex, Ey,
...
instrument.
\item {\bf RESTORE\_XRAY}$(index, x, y, z, kx, ky, kz, phi, Ex, Ey,
...
\item {\bf mcset\_ncount}$(n)$. Sets the number of x-ray histories to simulate to $n$.
\item {\bf mcget\_ncount}(). Returns the number of x-ray histories to simulate (usually set by option \verb+-n+).
\item {\bf mcget\_run\_num}(). Returns the number of x-ray histories that have been simulated until now.
\end{itemize}
...
\item {\bf inside\_rectangle}(\&$x$, \&$y$, $x$, $xw$, $yh$).
  ...
  Else return 0.
\item {\bf box\_intersect}(\&$l_1$, \&$l_2$, $x$, $y$, $z$, $k_x$, $k_y$, $k_z$,
  ...
  false otherwise.
\item {\bf cylinder\_intersect}(\&$l_1$, \&$l_2$, $x$, $y$, $z$, $k_x$, $k_y$, $k_z$,
  $r$, $h$).  Similar to {\bf box\_intersect}, but using a cylinder of height $h$ and radius $r$,
  centered at the origin.
\item {\bf sphere\_intersect}(\&$l_1$, \&$l_2$, $x$, $y$, $z$, $k_x$, $k_y$, $k_z$,
  $r$). Similar to {\bf box\_intersect}, but using a sphere
  of radius $r$.
\item {\bf ellipsoid\_intersect}(\&$l_1$, \&$l_2$, $x$, $y$, $z$, $k_x$, $k_y$, $k_z$,
  $a$,$b$,$c$,$Q$, ). Similar to {\bf box\_intersect}, but using an ellipsoid with half-axis $a$,$b$,$c$ oriented by the rotation matrix $Q$.
...
The $filename$ is first searched into the current directory (and all user additional locations specified using the \verb+-I+ option, see the 'Running \MCX\ ' chapter in the User Manual), and if not found, in the \verb+data+ sub-directory of the \verb+MCXTAS+ library location. \index{Library!Components!data}
\index{Environment variable!MCXTAS} This way, you do not need to have local copies of the \MCX\ Library Data files (see table~\ref{t:comp-data}).
...
%and {\bf Adapt\_check}. It performs adaptive importance sampling of x-rays for simulation efficiency optimization.
%Refer to these components for implementation examples, and ask the authors for more details.
%
