...
%\component{Monitor\_nD}{System, E. Farhi}{$x_\textrm{min}$, $x_\textrm{max}$, $y_\textrm{min}$, $y_\textrm{max}$, options}{$file$, $x_{width}, y_{height}, z_{depth}$, $bins$, $min$, $max$}{}
...
The input parameters for \textbf{Monitor\_nD} are its dimensions $x_{\rm
  min}, x_\textrm{max}, y_\textrm{min}$, $y_\textrm{max}$ (in meters) and an {\it
...
\item{(\textit{disk}) When choosing this geometry, the detector is a flat
    ...
    \begin{equation}
      \mbox{radius} = \max ( \mbox{abs } [ x_\textrm{min}, x_\textrm{max}, y_{\rm
...
names in the \textit{options} parameter. The data will be sorted into {\it
