...
for(;;) {
    if(($ARGV[0] eq "-m") || ($ARGV[0] eq "--multi")) {
      ...
      shift;
    } elsif(($ARGV[0] =~ /^-i([a-zA-ZæøåÆØÅ0-9_]+)$/) ||
          ($ARGV[0] =~ /^--inspect=([a-zA-ZæøåÆØÅ0-9_]+)$/)) {
      $inspect = $1;
      shift;
    } elsif($ARGV[0] =~ /^-ps$/) {
      $pg_devname = "cps";
      ...
      print "mcdisplay [-ps|-psc|-gif|-h|--multi|--zoom=<zoom factor> --inspect=<component name>] <simulation> <parameters>";
      exit;
    } else {
      last;
    }
...
my $global_device;
my $tmp_pg_devname = "mcdisplay.$pg_devname/$pg_devname"; 
if($pg_devname !~ "xserv") {
  if ($pg_devname =~ "cps") {
    $tmp_pg_devname = "mcdisplay.ps/cps";
  }
  $global_device = get_device($tmp_pg_devname);
}
else
...
while(!eof(IN)) {
    ...
    do {
      if($pg_devname !~ "xserv") {
          ...
          close(IN);
          pgclos;
          die "Wrote file '$tmp_pg_devname' ($pg_devname)\n";
      }
