...
(time gcc BNL_H8_simple.c -DRNG_ALG=1 -lm -O2 -o BNL_H8_simple_gcc_mt.out) &> compile_gcc_mt.txt

# 2 standard serial gcc-compilation, KISS
(time gcc BNL_H8_simple.c -DRNG_ALG=2 -lm -O2 -o BNL_H8_simple_gcc_KISS.out) &> compile_gcc_KISS.txt

# 3 MPICC with Mersenne Twister
(time mpicc BNL_H8_simple.c -DRNG_ALG=1 -lm -DUSE_MPI -O2 -lmpi -o BNL_H8_simple_mpicc_mt.out) &> compile_mpicc_mt.txt

# 4 MPICC with KISS
(time mpicc BNL_H8_simple.c -DRNG_ALG=2 -lm -DUSE_MPI -O2 -lmpi -o BNL_H8_simple_mpicc_KISS.out) &> compile_mpicc_KISS.txt

# 5 pgcc and -ta:multicore
(time pgcc -ta=multicore -Minfo=accel BNL_H8_simple.c -fast -DRNG_ALG=2 -DUSE_PGI -o BNL_H8_simple_multicore_KISS.out) &> compile_multicore_KISS.txt

# 5 pgcc and -ta=tesla...
(time pgcc -ta=tesla:cc70,managed,deepcopy -Minfo=accel -fast -DRNG_ALG=2 -DUSE_PGI -DNOSIGNALS BNL_H8_simple.c -o BNL_H8_simple_GPU_KISS.out) &> compile_GPU_KISS.txt
