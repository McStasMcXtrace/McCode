 ...
 * Date:       Thu Nov 21 14:06:18 2019
 * File:       mini.c
 * CFLAGS=
 */

#define MCCODE_STRING "McStas 3.0-dev - Nov. 20, 2019"
...
// rearrange an array of particles such as the absorbed ones appear last
...
/* init variables and counters for TRACE */
  ...
  for (unsigned long long cloop=0; cloop<loops; cloop++) {
    ...
    // put absorbed particles last in the particle
    innerloop = sort_absorb_last(particles, innerloop);
    printf("%ld\n", innerloop);
...
_class_PSD_monitor *class_PSD_monitor_save(_class_PSD_monitor *_comp
  ...
  #define PSD_p (_comp->_parameters.PSD_p)
  #define PSD_p2 (_comp->_parameters.PSD_p2)
  SIG_MESSAGE("[_detector_save] component detector=PSD_monitor() SAVE [/usr/share/mcstas/3.0-dev/monitors/PSD_monitor.comp:117]");
  DETECTOR_OUT_2D(
    "PSD monitor",
    "X position [cm]",
    "Y position [cm]",
...
_class_PSD_monitor *class_PSD_monitor_finally(_class_PSD_monitor *_comp
  ...
  #define PSD_p (_comp->_parameters.PSD_p)
  #define PSD_p2 (_comp->_parameters.PSD_p2)
  SIG_MESSAGE("[_detector_finally] component detector=PSD_monitor() FINALLY [/usr/share/mcstas/3.0-dev/monitors/PSD_monitor.comp:129]");
...
_class_PSD_monitor *class_PSD_monitor_display(_class_PSD_monitor *_comp
  ...
  #define PSD_p (_comp->_parameters.PSD_p)
  #define PSD_p2 (_comp->_parameters.PSD_p2)
  SIG_MESSAGE("[_detector_display] component detector=PSD_monitor() DISPLAY [/usr/share/mcstas/3.0-dev/monitors/PSD_monitor.comp:136]");
  printf("MCDISPLAY: component %s\n", _comp->_name);
  multiline(5,
    (double)xmin, (double)ymin, 0.0,
    (double)xmax, (double)ymin, 0.0,
