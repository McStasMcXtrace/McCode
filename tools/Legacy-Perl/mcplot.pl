...
if ($ENV{"HOME"} && -e $ENV{"HOME"}."/.".$MCSTAS::mcstas_config{'MCCODE'}."/".$MCSTAS::mcstas_config{'VERSION'}."/mccode_config.perl") {
  print "mcplot: reading local $MCSTAS::mcstas_config{'MCCODE'} configuration from " . $ENV{"HOME"}."/.".$MCSTAS::mcstas_config{'MCCODE'}."/".$MCSTAS::mcstas_config{'VERSION'}."/mccode_config.perl\n";
...
sub pgplotit {
  ...
  if ($daemon eq 0) {
	  ...
	  if ($global_open == 0) {
	      ...
	      system("$MCSTAS::mcstas_config{'PLOTCMD'} --format=McStas $passed_arg_str -$dev $file");
              next;
          }
	      ...
	      system("$MCSTAS::mcstas_config{'PLOTCMD'} --format=McStas $passed_arg_str $filename -$dev");
          }
