...
# Enhanced mxdisplay script for --trace output from McXtrace simulation.
#
...
#   Copyright (C) 1997-20014, All rights reserved
...
if ($ENV{"HOME"} && -e $ENV{"HOME"}."/.".$MCSTAS::mcstas_config{'MCCODE'}."/".$MCSTAS::mcstas_config{'VERSION'}."/mccode_config.perl") {
  print "mxdisplay: reading local $MCSTAS::mcstas_config{'MCCODE'} configuration from " . $ENV{"HOME"}."/.".$MCSTAS::mcstas_config{'MCCODE'}."/".$MCSTAS::mcstas_config{'VERSION'}."/mccode_config.perl\n";
...
sub read_instrument {
    ...
    while(<$in>) {
	    ...
	    if ($MCSTAS::mcstas_config{'PLOTTER'} =~ /mantid/i) {
	      ...
	      write_process("<!-- IDF generated using McXtrace MxDisplay and the Mantid backend -->\n");
...
WorldInfo {
  ...
  info [ \"URL:    http://www.mcxtrace.org/\"
    \"Editor: mxdisplay @argv\"
    \"Creator:$sim_cmd simulation (McXtrace)\" ]
}
Viewpoint {
  description \"Default\"
   position 0 0.2 -1
	    ...
	    if ($MCSTAS::mcstas_config{'PLOTTER'} =~ /mantid/i) {
            ...
            if ($MCSTAS::mcstas_config{'PLOTTER'} =~ /Matlab/i) {
              ...
              if ($save) {
                ...
                write_process("h=figure('numbertitle','off','name','$sim McXtrace Instrument')\n;");
...
sub read_neutron {
    ...
    while(<$in>) {
        if($st == 0 && /^ENTER:/) {
            ...
            # State after leaving - should probably be removed in McXtrace.
            next;
...
sub plot_neutron {
    ...
    if ($MCSTAS::mcstas_config{'PLOTTER'} =~ /McXtrace|PGPLOT/i) {
      # PGPLOT only
      PGPLOT::pgsci(3);
...
die "Usage: mxdisplay [-mzipfh][-gif|-ps|-psc] Instr.out [instr_options] params
 ...
 SEE ALSO: mxtrace, mxdoc, mxplot, mxrun, mxgui
...
if ($plotter =~ /scriptfile/i && not $file_output) {
  $file_output="mxdisplay_commands";
...
if ($plotter =~ /McXtrace|PGPLOT/i && $MCSTAS::mcstas_config{'PGPLOT'} eq "no") {
  print STDERR "\n******************************************************\n";
...
if ($plotter =~ /McXtrace|PGPLOT/i) { # PGPLOT is plotter!
  if ($int_mode == 1)
    {
      my $ext  = "ps";
      my $type = "ps";
...
if ($int_mode == 0 && $plotter =~ /McXtrace|PGPLOT/i)  { printf STDERR "McDisplay/PGPLOT: Press H key for help.\n"; }
