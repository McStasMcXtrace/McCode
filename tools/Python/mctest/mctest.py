...
def mccode_test(branchdir, testdir, limitinstrs=None, instrfilter=None):
    ...
    for test in tests:
        ...
        if os.path.exists(binfile):
            ...
            if test.testnb > 0 or (not args.skipnontest):
                log = LineLogger()
                t1 = time.time()
                cmd = "%s --info %s &> compile_stdout.txt" % (mccode_config.configuration["MCRUN"],test.localfile)
    ...
    for test in tests:
        ...
        if mpi is not None:
            if openacc is True:
                cmd = "%s -s 1000 %s %s -n%s --openacc --mpi=%s -d%d &> run_stdout_%d.txt" % (mccode_config.configuration["MCRUN"],test.localfile, test.parvals, ncount, mpi, test.testnb, test.testnb)
            else:
                cmd = "%s -s 1000 %s %s -n%s --mpi=%s -d%d &> run_stdout_%d.txt" % (mccode_config.configuration["MCRUN"],test.localfile, test.parvals, ncount, mpi, test.testnb, test.testnb)
        else:
            cmd = "%s -s 1000 %s %s -n%s -d%d  &> run_stdout_%d.txt" % (mccode_config.configuration["MCRUN"],test.localfile, test.parvals, ncount, test.testnb, test.testnb)
