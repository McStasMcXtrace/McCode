...
class McStas:
    ...
    def prepare(self, options):
        ...
        if not options.force_compile and existingC is not None:
            ...
            if not options.no_main:
                if self.options.I is not None:
                    Process(options.mccode_bin).run(['-t','-o', self.cpath, self.path, '-I', self.options.I])
                else:
                    Process(options.mccode_bin).run(['-t','-o', self.cpath, self.path])
            else:
                if self.options.I is not None:
                    Process(options.mccode_bin).run(['--no-main','-o', self.cpath, self.path, '-I', self.options.I])
                else:
                    Process(options.mccode_bin).run(['--no-main','-o', self.cpath, self.path])
    ...
    def run(self, pipe=False, extra_opts=None, override_mpi=None):
        ...
        proxy_opts_val.extend(('literal-defined', 'literal-type', 'literal'))
        ...
        proxy_opts_flags = ['trace', 'no-output-files', 'info', 'list-parameters', 'literal-list']
