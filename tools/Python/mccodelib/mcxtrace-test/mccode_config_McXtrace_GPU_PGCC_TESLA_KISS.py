...
'''
mcxtrace/mcxtrace configuration.
'''
configuration = {
    "MCCODE_VERSION": '3.0-test',
    "MCCODE_LIB_DIR": '/usr/share/mcxtrace/3.0-test',
    "MCCODE": 'mcxtrace',
    "MCRUN": 'mxrun',
    "MCPLOT": 'mxplot-pyqtgraph',
    "MCDISPLAY": 'mxdisplay-webgl',
    "TOOL_NAME": 'mxgui',
    "PARTICLE": 'xray',
    "BROWSER": 'xdg-open',
    "GUICOLS": '3',
...
'''
compilation = {
    "CFLAGS": '-ta=tesla:cc70,managed,deepcopy -Minfo=accel -DOPENACC -DNOSIGNALS -DRNG_ALG=2',
...
def check_env_vars():
    ...
    # MCCODE_LIB_DIR
    if not os.getenv('MCXTRACE_OVERRIDE') is None:
        configuration['MCCODE_LIB_DIR'] = os.getenv('MCXTRACE_OVERRIDE')
    # CFLAGS
    if not os.getenv('MCXTRACE_CFLAGS_OVERRIDE') is None:
        compilation['CFLAGS'] = os.getenv('MCXTRACE_CFLAGS_OVERRIDE')
    # CC
    if not os.getenv('MCXTRACE_CC_OVERRIDE') is None:
        compilation['CC'] = os.getenv('MCXTRACE_CC_OVERRIDE')
    # MPICC
    if not os.getenv('MCXTRACE_MPICC_OVERRIDE') is None:
        compilation['MPICC'] = os.getenv('MCXTRACE_MPICC_OVERRIDE')
