...
class LDAPDataPopulator:
    def __init__(self, cn, pw):
        import base64
        print "In LDAPDataPopulator\n"
        self.data = LDAPData()
        conn = LDAPComm.LDAPComm()
        query = "cn=\"%s\"" % cn        
        dn = "cn=%s,ou=person,dc=fysik,dc=dtu,dc=dk" % cn

        print "Passing %s to LDAPComm::ldapQuery to query %s\n"%(dn,query) # see ldapcomm.py
        
        usr_details = conn.ldapQuery(dn, pw, query)
    
        for line in usr_details:
            if 'uid' in line:
                self.data.setuid(base64.standard_b64decode(split(" ", line)[1]))
            if 'cn' in line:
                self.data.setcn(base64.standard_b64decode(split(" ", line)[1]))
            if 'sn' in line:
                self.data.setsn(base64.standard_b64decode(split(" ", line)[1]))
            if 'email' in line:
                self.data.setmail(base64.standard_b64decode(split(" ", line)[1]))
            if 'displayname' in line:
                self.data.setdisplayname(base64.standard_b64decode(split(" ", line)[1]))
