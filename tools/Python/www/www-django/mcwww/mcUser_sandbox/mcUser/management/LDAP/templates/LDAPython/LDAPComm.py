...
#===============#
    def ldapQuery(self, auth_dn, auth_pw, query):
        ...
        pipe = PIPE
        self.query_num += 1
        log_str = cn + " QUERY with: ldapsearch -LLL -b dc=DN -D" + auth_dn + "-w PASSWORD " + query+"\n"
        self.log(log_str)
        try:
            fid  = Popen(["ldapsearch", "-LLL", "-b", "dc=DN", "-D", auth_dn, "-w", auth_pw, query],
                         stdout=pipe,
                         stderr=pipe)
...
#==========================#
    def ldapAdminGroupQuery(self, auth_cn):
        ...
        self.log("%s AUTHORITY ACCESS QUERY with: ldapsearch -LLL -b ou=groups,dc=DN -D cn=DummyUser,ou=person,dc=DN -w DummyPW %s\n" % (cn, query) )
        pipe = PIPE
        try:
            fid = Popen(
                ["ldapsearch", "-LLL", "-b", "dc=DN", "-D", "cn=DummyUser,ou=person,dc=DN", "-w", "DummyPW", query],
                stdout=pipe,
                stderr=pipe)
            stdout,stderr = fid.communicate()
            bill = stdout
...
#=====================#
    def ldapAuthenticate(self, auth_cn, auth_pw):
        dn = "cn=%s" + auth_cn + "ou=person,dc=DN"
        try:
