...
def get_json_2d(xmin, xmax, ymin, ymax, image_str, xlabel, ylabel, title):
    '''  '''
    params = {}
    p = params
...
def main(args):
    ...
    if type(node) is PNSingle:
        ...
        if type(data) is Data1D:
            ...
            encoded = str(base64.b64encode(contents)).lstrip('b').strip("\'")
            ...
            json_str = get_json_2d(xmin, xmax, ymin, ymax, encoded, data.xlabel, data.ylabel, data.title)
