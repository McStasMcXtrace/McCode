...
class IFitInterface:
    ''' The lowest level above matlab, with special functions for getting information out of iData and iFinc objects '''
    def __init__(self):
...
class IFitLoaderSimple:
    ''' loads 1D and 2D data from ifit and transforms this into a usable plotgraph '''
    def __init__(self, simfile):
        '''  '''
        self.simfile = simfile[0]
        self.plot_graph = None
        self.interface = IFitInterface()
        if not os.path.exists(self.simfile):
    ...
    def load(self):
        ...
        i.assign('a', 'load(iData, \'%s\')' % self.simfile)
...
def main(args):
    ...
    try:
        # load data
        loader = IFitLoaderSimple(simfile=args.simulation)
        graph = loader.load()
