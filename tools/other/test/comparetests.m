function [results,instruments,ref,datasets]=comparetests(testdir,ref,normalise)
...
if not(isempty(ref.x_meta.mpi))
    normalisation=ref.x_meta.mpi;
else
        normalisation=1;
end
            ...
            if not(refinst.testval==-1)
                for l=1:length(datasets)
                    if isfield(datasets{l},instruments{k})
                        ...
                        if not(isempty(thisinst.runtime)) && isnumeric(thisinst.runtime)
                            results(k,l)=refinst.runtime/thisinst.runtime;
                        else
                            results(k,l)=NaN;
                        end
                    else
...
h=ylabel({['Relative speedup wrt. ' num2str(normalisation) 'x'];ref.x_meta.cpu_type});
set(h,'interpreter','none');
h=title(['Relative speedup wrt. ' refname]);
set(h,'interpreter','none');
set(gca,'yscale','log');

eval(['print -dpng ' testdir '.png']);